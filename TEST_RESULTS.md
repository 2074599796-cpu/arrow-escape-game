...
GUI 动画专项冒烟测试：先自动点击第一关被阻挡的向右飞剑，确认飞剑回弹后仍在原位、剑心由 3 减为 2；再点击无阻挡飞剑，确认它加速飞出棋盘并从状态中消失。图片飞剑版已在 Windows 桌面实际启动并完成视觉检查。测试输出：

```text
GUI smoke test OK: collision bounce and accelerated flying-sword exit both finished
```
